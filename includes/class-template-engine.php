...
class Template_Engine {
	...
	public static function special_tags() {
		return array(
			'all_fields'  => __( 'All submitted fields as a list', 'b418-tg-cf7' ),
			'_form_title' => __( 'Form title', 'b418-tg-cf7' ),
			'_site_title' => __( 'Site title', 'b418-tg-cf7' ),
			'_site_url'   => __( 'Site URL', 'b418-tg-cf7' ),
			'_page_url'   => __( 'Page the form was submitted from', 'b418-tg-cf7' ),
			'_date'       => __( 'Submission date', 'b418-tg-cf7' ),
			'_time'       => __( 'Submission time', 'b418-tg-cf7' ),
			'_user_ip'    => __( 'Visitor IP address', 'b418-tg-cf7' ),
			'_user_agent' => __( 'Visitor browser (user agent)', 'b418-tg-cf7' ),
		);
	}
	...
	public static function render_sample( $body, $form, $parse_mode = 'none' ) {
		...
		if ( $form && method_exists( $form, 'scan_form_tags' ) ) {
			foreach ( $form->scan_form_tags() as $tag ) {
				...
				$fields[ $tag->name ] = sprintf( __( '(sample value for “%s”)', 'b418-tg-cf7' ), $tag->name );
			}
		}

		$specials = array(
			'all_fields'  => self::format_all_fields( $fields, 'none' ),
			'_form_title' => $form ? $form->title() : __( 'Sample form', 'b418-tg-cf7' ),
			...
			'_user_ip'    => '127.0.0.1',
			'_user_agent' => __( '(test message)', 'b418-tg-cf7' ),
		);
